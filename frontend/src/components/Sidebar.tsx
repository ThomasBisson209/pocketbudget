import { LayoutDashboard, ArrowLeftRight, CreditCard, PiggyBank, LogOut, X } from 'lucide-react';
    ...
    <aside className="w-60 shrink-0 bg-green-900 text-white flex flex-col h-full">
      ...
      <div className="px-4 py-4 border-t border-green-800">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-8 h-8 rounded-full bg-green-700 flex items-center justify-center text-sm font-semibold">
            {username?.[0]?.toUpperCase() ?? '?'}
          </div>
          <span className="text-sm text-green-100 truncate">{username}</span>
        </div>
        <button
          onClick={onLogout}
