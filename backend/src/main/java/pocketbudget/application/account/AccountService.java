...
public class AccountService {
    ...
    public BalanceHistoryDto getBalanceHistory(String accountId, int month, int year) {
        Account account = accountRepository.findById(new AccountId(accountId))
            .orElseThrow(() -> new AccountNotFoundException(accountId));

        double currentBalance = account.getBalance();
        List<Transaction> allTx = transactionRepository.findByAccountId(accountId);

        LocalDate firstDay = LocalDate.of(year, month, 1);
        LocalDate lastDay = firstDay.withDayOfMonth(firstDay.lengthOfMonth());

        // Net effect of transactions AFTER the selected month
        ...
        // Net effect of transactions IN the selected month
        double netInMonth = allTx.stream()
            .filter(t -> !t.getDate().isBefore(firstDay) && !t.getDate().isAfter(lastDay))
            .mapToDouble(t -> t.getType() == TransactionType.CREDIT ? t.getAmount() : -t.getAmount())
            .sum();

        // Balance at the start of the month (before any transaction in this month)
        double startBalance = currentBalance - netAfter - netInMonth;

        // Group in-month transactions by date
        ...
        log.info("Balance history fetched: accountId={}, month={}/{}", accountId, month, year);
